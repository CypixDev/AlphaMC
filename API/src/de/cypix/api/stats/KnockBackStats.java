...
public class KnockBackStats {
    ...
    public KnockBackStats(Player p){
        this.p = p;
    }

    public void loadStats(){
        if(isInList(p)) {
            Mysql.update("UPDATE KnockBack SET Player='"+p.getName()+"' WHERE UUID='"+p.getUniqueId()+"'");

            ResultSet rs = Mysql.getResult("SELECT * FROM KnockBack WHERE UUID='" + p.getUniqueId() + "'");
    ...
    public void sendStatsMessage(){
        if(stats.containsKey(p)) {
            ...
            p.sendMessage("§8➤ §7Deaths §b: " + list.get(3));
            p.sendMessage("§8➤ §7Höchster KillStreak §b: " + list.get(4));
            p.sendMessage(" ");
            p.sendMessage("§7§l┗╋§7§l§m----------§7§l◥◣◆◢◤§7§l§m----------§7§l╋┛");


        }else{
            p.sendMessage("Du hast KnockBack noch nicht gespielt !");
        }
    }

    private static boolean isInList(Player p){
        ResultSet set = Mysql.getResult("SELECT * FROM KnockBack WHERE UUID='"+p.getUniqueId()+"'");
        try{
            return set.next();
        }catch(SQLException e1){
            e1.printStackTrace();
