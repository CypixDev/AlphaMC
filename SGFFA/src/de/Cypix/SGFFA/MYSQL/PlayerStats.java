...
public class PlayerStats {
    ...
    public static void saveStats(Player p){
        if(MYSQL.isConnected()){
            MYSQL.update("UPDATE sgffa SET Kills='"+kills.get(p)+"' WHERE UUID='"+p.getUniqueId()+"'");
            MYSQL.update("UPDATE sgffa SET Deaths='"+new PlayerStats(p).getDeaths()+"' WHERE UUID='"+p.getUniqueId()+"'");
            MYSQL.update("UPDATE sgffa SET KillStreak='"+killstreak.get(p)+"' WHERE UUID='"+p.getUniqueId()+"'");
        }
    }
    ...
    public static void addPlayer(Player p){
        if(!isInList(p)){
            MYSQL.update("INSERT INTO sgffa (Player , UUID, Kills, Deaths, KillStreak) VALUES ('"+p.getName()+"', '"+p.getUniqueId()+"', '0', '0', '0')");
        }else{
    ...
    public static void loadStats(Player p){
        ...
        p.sendMessage(Var.pr+"§cDeine Stats wurden geladen!");
    }
    private static int getKills(Player p){
        ResultSet set = MYSQL.getResult("SELECT * FROM sgffa WHERE UUID='"+p.getUniqueId()+"'");
        try{
            while(set.next()){
    ...
    private static void updateName(Player p){
        MYSQL.update("UPDATE sgffa SET Player='"+p.getName()+"' WHERE UUID='"+p.getUniqueId()+"'");
    }
}
