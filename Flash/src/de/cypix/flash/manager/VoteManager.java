    ...
    public static void vote(String vote, Player p){
        if(voted.containsKey(p)){
            if(voted.get(p).equalsIgnoreCase(vote)){
                p.sendMessage(Main.pr+"Dafür hast du schon abgestimmt !");
            }else{
                int e = map.get(voted.get(p));
                map.put(voted.get(p), e-1);
                voted.put(p, vote);
                map.put(vote, map.get(vote)+1);
                p.sendMessage(Main.pr+"Du hast deine Stimme auf "+vote+" geändert !");
            }
        }else{
            map.put(vote, map.get(vote)+1);
            voted.put(p, vote);
            p.sendMessage(Main.pr+"Du hast für "+vote+" gevoted !");
